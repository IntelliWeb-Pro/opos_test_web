...
# --- VISTA PARA LAS ESTADÍSTICAS DEL USUARIO ---
class EstadisticasUsuarioView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if not resultados.exists():
            return Response({
                "error": "No hay datos suficientes para generar estadísticas."
            }, status=status.HTTP_404_NOT_FOUND)

        media_general = resultados.aggregate(
        ...
        stats_tema = resultados.values('tema__nombre', 'tema__oposicion__nombre').annotate(
        ...
        data = {
            ...
            "stats_por_oposicion": [
                {
                    "oposicion": item['tema__oposicion__nombre'],
                    "media": round(item['media_oposicion'], 2)
                } for item in stats_oposicion
            ],
            "stats_por_tema": [
                {
                    "oposicion": item['tema__oposicion__nombre'],
                    "tema": item['tema__nombre'],
                    "media": round(item['media_tema'], 2)
                } for item in stats_tema[:10]
            ]
        }
